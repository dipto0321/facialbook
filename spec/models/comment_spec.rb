...
RSpec.describe Comment, type: :model do
  ...
  context 'Factories and instance method' do
    it 'has a valid factory' do
      ...
      expect(build(:post_comment, commentable_id: post.id, commenter_id: user.id)).to be_valid
    end

    it 'has not a valid comment without body' do
      @user = create(:user)
      @post = create(:user_post, postable_id: @user.id, author_id: @user.id)
      @comment = build(:post_comment, commentable_id: @post.id, commenter_id: @user.id, body: nil)
      @comment.valid?
